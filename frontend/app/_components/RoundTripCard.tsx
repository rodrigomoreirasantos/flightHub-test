...
interface RoundTripCardProp {
  tripDetail: RoundTripData;
  dateRoundTrip: DateRange | undefined;
}

const RoundTripCard = ({ tripDetail, dateRoundTrip }: RoundTripCardProp) => {
  return (
    <Card>
        ...
        <div className="text-gray-600 h-7 bg-gray-300 rounded-t-md flex justify-between items-center container">
          ...
          {dateRoundTrip ? (
            <p className="text-xs font-medium flex">
              <PlaneTakeoff size={16} className="mr-1" />
              {`${format(dateRoundTrip.from, "PPP")} -`}
              <PlaneLanding size={16} className="ml-1 mr-1" />
              {` ${format(dateRoundTrip.to, "PPP")}`}
            </p>
          ) : (
            "Date not available"
          )}
